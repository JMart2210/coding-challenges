...
const cost = (books) => {
  if (books.length < 2) return books.length * 800;
  let cache = {};
  while (books.length > 0) {
    let set = [];
    for (let i = 0; i < books.length; i++) {
      if (set.length === 5) break;
      if (!set.includes(books[i])) {
        set.push(books[i]);
        books.splice(i, 1);
        i--;
      }
    }
    cache[set.length] = cache[set.length] ? cache[set.length] + 1 : 1;
  };
  while (cache[5] && cache[3]) {
    cache[5]--
    cache[3]--
  ...
  for (let key in cache) {
    if (key === '5') {
      total += cache[key] * 3000;
    }
    if (key === '4') {
      total += cache[key] * 2560;
    }
    if (key === '3') {
      total += cache[key] * 2160;
    }
    if (key === '2') {
      total += cache[key] * 1520;
    }
    if (key === '1') {
      total += cache[key] * 800;
    }
